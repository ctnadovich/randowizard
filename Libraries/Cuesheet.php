 ...
 */
	...
	public function draw_cuesheet_pages($edata)
	{
		...
		$this->font_bigprint();
		$this->font_italic();
		$this->Cell($w, 2 * $this->baseline_skip, $edata['this_organization'], 0, 2, 'C');
		$this->font_bold();
		...
		$this->font_plain();
		$header = $this->header_text_array($edata);
		...
		if ($this->cue_abbreviations != []) {
			$glossary = implode(', ', array_map(function ($v, $k) {
				return "$k:$v";
			}, $this->cue_abbreviations, array_keys($this->cue_abbreviations)));
			$this->font_normalprint();
			$this->font_italic();
			$y_now = $this->GetY();
			$this->SetXY($x, $y_now);
		...
		$this->render_cues($full_width, $h, $this->cue_text_array($edata['cues'], $edata['route_controles']), $text_columns);
	}
	...
	public function cue_text_array($cues, $controles)
	{
		...
		foreach ($cues as $cue) {
			...
			if ($cue['t'] == 'Control') {
				...
				$cue_lines[] = [
					$controle_i + 1, $controle_str, $style_text, $open_time_str, $close_time_str,
