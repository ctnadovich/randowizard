 ...
 */    
        ...
        public function __construct($params=[]) { // , $orientation="P", $unit="in", $size="letter") {
  ...
  */   
    ...
    // 				}
            ...
            // for($c=0; $c<$nc; $c++){ // for each card
                ...
                $last_height=$baseline_skip;
                    ...
                    foreach($lines as $line){ // for each line
                        ...
                        $text=(isset($line['text']))?utf8_decode($line['text']):''; //$text = "($x)$text";
        ...
        private function back_body_text($route_event, $rider=null){
            ...
            $street=(isset($rider->m_street))?($rider->m_street):'';
            $city=(isset($rider->m_city))?$rider->m_city:'';
            $state=(isset($rider->m_state))?$rider->m_state:'';
            $zip=(isset($rider->m_zip))?$rider->m_zip:'';
            $country=(isset($rider->m_country))?$rider->m_country:'';
            
            $start_code=(!empty($rider->rusa_id))?$this->CI->model_parando->make_start_code($this->event,$rusa_id):'';
            
            
            
        if(empty($street.$city.$state.$zip.$country)){
                $address1='Adddress:'; $address2='';
            }else{
            ...
            switch($event_type){
                case 'acp': $cert='Homologation'; break;
                case 'rusa': $cert='Certification'; break;
                default: $cert='Validation'; break;
            }
        ...
        private function controle_body_text($controle){
            ...
            $dzaddress=$this->delete_zipcode($ca['address']);
                ...
                foreach($controles as $controle){
                    ...
                    if($style_text=='staffed' ||$style_text=='overnight' || $style_text=='merchant' || $style_text=='open'){  // Draw stamp region for Timed Controles
                        ...
                        if(filter_var($start_stamp, FILTER_VALIDATE_URL) && $n==1){
                                    $this->font_resize(); $this->font_bold();
