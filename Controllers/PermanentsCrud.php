...
class PermanentsCrud extends BaseController
{
    ...
    {
        $permModel = model('Event');
        $now = (new \DateTime('now', (new \DateTimeZone('UTC'))))->format('Y-m-d H:i:s');
        $id = $stateParameters->primaryKeyValue;
        $permModel->update($id, ['last_changed' => "$now"]);
...
EOT;
    ...
    public function roster_upload($event_code)
    {
        ...
        $event = $this->permModel->eventByCode($event_code);

        $this->viewData['errors'] = [];
        $this->viewData['event_code'] = $event_code;
        $this->viewData['event_name_dist'] = $this->permModel->nameDist($event);
    ...
    private function processRosterFile($uploadedFile, $event)
    {
        ...
        $event_cutoff_datetime = $this->permModel->getCutoffDatetime($event);
